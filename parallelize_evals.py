...
class GPUJobQueue:
    ...
    def process_queue(self):
        while True:
            if self.job_queue.empty() and all(proc is None for proc in self.running_processes):
                break

            for gpu_id in range(self.num_gpus):
                if self.running_processes[gpu_id] is None and self.gpu_locks[gpu_id].acquire(blocking=False):
                    if not self.job_queue.empty():
                        bash_command = self.job_queue.get()
                        threading.Thread(target=self.run_job, args=(gpu_id, bash_command)).start()
                        self._save_queue()
                    else:
                        self.gpu_locks[gpu_id].release()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_names", nargs='+', help="YAML configuration files that need to be evaluated", required=True)
    parser.add_argument(
        "--tasks",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--log_dir", default="eval_job_logs", help="Directory for job logs")
    ...
    for config in args.config_names:
        if not config.endswith(".yaml"):
            config = config+".yaml"
        assert os.path.join(os.path.abspath(__file__), "cache_configs", config), f"{config} not found in cache_configs"
