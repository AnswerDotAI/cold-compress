...
def main():
    parser = argparse.ArgumentParser(
        description="Script to remove all but the first K layers of model. For Debugging."
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=Path,
        default=Path("checkpoints/meta-llama/Meta-Llama-3-8B-Instruct"),
    )
    ...
    trunc_dir = args.checkpoint_dir.with_name(
        args.checkpoint_dir.name + f"-{args.trunc_layers}-Layers"
    )
    ...
    new_weights = dict(
        {
            ...
            if keep_weight(key, args.trunc_layers)
        }
    )
    torch.save(new_weights, trunc_dir / "model.pth")

    orig_size = sum([value.numel() for value in weights.values()])

    new_size = sum([value.numel() for value in new_weights.values()])

    print(
        f"Reduced number of parameters from {orig_size} to {new_size} by truncating to first {args.trunc_layers} layers."
    )
