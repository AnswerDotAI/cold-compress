...
def hf_download(repo_id: Optional[str] = None, hf_token: Optional[str] = None) -> None:
    ...
    if os.listdir(f"checkpoints/{repo_id}"):
        print(f"Directory checkpoints/{repo_id} is not empty, skipping download. First, \"rm -rf checkpoints/{repo_id}\" if you want to re-download.")
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--repo_id', type=str, default="karpathy/tinyllamas", help='Repository ID to download from.')
