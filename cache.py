...
class KVCacheHeavyHitters(KVCacheWindow):
    relevant_kwargs = ["max_cache_length", "global_tokens", "history_attn_window", "eviction_num"]

    def __init__(
        self, max_batch_size, n_heads, head_dim, dtype=torch.bfloat16, **kwargs
    ):
        super().__init__(max_batch_size, n_heads, head_dim, dtype, head_specific=True, **kwargs)

        # Initialize a buffer for the attention histories
        history_shape = (max_batch_size, n_heads, self.max_cache_length, self.history_attn_window)
        self.register_buffer("attn_history", torch.zeros(history_shape, dtype=dtype))
        self.attn_counter = 0

        # Different eviction queue for each attention head
        self.eviction_queue = [[] for _ in range(n_heads)]

        # TODO Do we want to make this a kwarg? Will need to rename since "max_cache_length" means window size for KVCacheWindow
    ...
    def return_attn(self) -> bool:
        ...
        """
        return not self.is_prefill() and len(self.eviction_queue[0]) <= self.history_attn_window

    def update_attn_history(self, attn):
        # assert that the query length is 1
        assert attn.shape[2] == 1
        attn = attn.squeeze(0).squeeze(1)
        keys = attn.shape[1]
        attn_is_low = (attn < 1 / keys).int()
        self.attn_history[:, :, :keys, self.attn_counter % self.history_attn_window] = attn_is_low
        self.attn_counter += 1
    ...
    def refill_eviction_queue(self, input_pos: int):
        ...
        _, unimportant_toks = unimportant_cts.topk(self.eviction_num, dim=-1, largest=True)
    ...
    def _update(self, input_pos, k_val, v_val):
        ...
        if min(fill_indices) == max(fill_indices):  # Faster to do a bulk assign
            fill_indices = [fill_indices[0]]
            self.fill(fill_indices=fill_indices, input_pos=input_pos, k_val=k_val, v_val=v_val)
            self.attn_history[:, :, fill_indices, :] = 0
        else:
